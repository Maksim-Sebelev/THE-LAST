...
Эта функция вызывается в read. Поэтому если в единице траснялции специализированы шаблоны для нужных нод, то они и будут использованы.\
...
В библиотеке есть функции write и read, предназначенные для записи/чтения в/из специальный(ого) формат(а) дерева. Для каждого модуля можете сделать специализации функции create, которые из под себя вызывает read. Таким образом, ast можно перемещать между модулями программы через файл со специальным текстовым форматом. Согласен, реализовывать create в каждом модуле может быть довольно раздражающе.\ 
Есть случаи, когда можно обойтись малой кровью: например если на весь модуль вам нужна единая сигнатура для всех нод, то достаточно сделать 
...
Одако при использовании функции dump для ast с явной генерацией изображения будет вызыван graphviz. Об это читайте далее.
