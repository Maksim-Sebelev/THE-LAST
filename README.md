...
Как и обещал, возвращаемся к read. Данная функция предназначена для чтения специального тектового представления дерева. Как Вы помните, для построения ноды нужно указывать нужные сигнатуры visit. Так как read библиотечная функция, то она как то должна получать нужную специализацию функции create. Поэтому модуль ast_read следует импортировать после специализаций create. Да, это довольно сомнительное решение. Но для первой версии библиотеки это рабочее решение. В дальнейшем будем думать что с этим сделать.
